 ...
 */
export function formatTaskML(content: string, options: FormatOptions = {}): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Blank line handling is done above when detecting sections
...
export function sortByPriority(content: string): string {
  // This is a more advanced feature - just return formatted for now
  return formatTaskML(content);
}
